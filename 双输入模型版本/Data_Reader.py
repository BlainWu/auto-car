...
def train_mapper(sample):
    ...
    try:
        speed = np.ones([25]) * float(vel)
        speed = np.array(speed).astype(np.float32)
        img = Image.open(img_path) #(240,320)
        # 统一图片大小
        img = img.resize((resize_size, resize_size), Image.ANTIALIAS) #(128,128)
        # 把图片转换成numpy值
        ...
        print("%s 该图片错误，请删除该训练图片并重新创建图像数据列表" % img_path)
...
def test_mapper(sample):
    ...
    img = img[(2, 1, 0), :, :] / 255.0
    #speed = np.array(speed).astype(np.float32)
    #image_speed = np.concatenate((img, speed), axis=2)  # 输出(3, 128, 129)
