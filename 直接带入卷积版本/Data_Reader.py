...
def train_mapper(sample):
    ...
    try:
        speed = np.ones([3, 128, 1]) * (float(vel)/2200.0)
        img = Image.open(img_path) #(240,320)
        # 统一图片大小
        img = img.resize((resize_size-1, resize_size), Image.ANTIALIAS) #(128,127)
        # 把图片转换成numpy值
        ...
        image_speed = np.concatenate((img, speed), axis=2)  # 输出(3, 128, 128)
...
def test_mapper(sample):
    ...
    img = Image.open(img)
    # 统一图像大小
    img = img.resize((crop_size-1, crop_size), Image.ANTIALIAS)
