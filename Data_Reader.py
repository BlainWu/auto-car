...
def train_mapper(sample):
    img_path, speed , label, crop_size, resize_size = sample
    try:
...
def train_reader(train_list_path, crop_size, resize_size):
    def reader():
        with open(train_list_path, 'r') as f:
            ...
            # 开始获取每张图像和标签
            for line in lines:
                img, speed, label = line.split('\t')[0],line.split('\t')[1],line.split('\t')[2] #取第一列（图片地址）和第三列（角度）
                yield img , speed, label, crop_size, resize_size
...
def test_mapper(sample):
    img, speed , label, crop_size = sample
    img = Image.open(img)
    # 统一图像大小
...
def test_reader(test_list_path, crop_size):
    def reader():
        with open(test_list_path, 'r') as f:
            ...
            for line in lines:
                img, speed, label = line.split('\t')[0],line.split('\t')[1],line.split('\t')[2] #取第一列（图片地址）和第三列（角度）
                yield img,speed, label, crop_size
    #参数1：数据映射函数，reader是产生数据的reader，cpu_count()按照满cpu核读取，待读取队列大小
