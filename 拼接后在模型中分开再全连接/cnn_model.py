...
def cnn_model(image_speed):
    image = fluid.layers.crop(image_speed, shape=[-1,3, 128, 128])
    speed = fluid.layers.crop(image_speed, shape=[-1,3, 128, 1],offsets = [-1,3, 128, 128])
    speed = fluid.layers.flatten(x=speed, axis=1)
    speed = fluid.layers.slice(input = speed, axes=[1], starts=[0], ends=[25])
