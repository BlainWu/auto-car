...
def image_load(video):
    ...
    img = img.resize((128, 128), Image.ANTIALIAS)  # (128,128)

    img = np.array(img).astype(np.float32)
    img = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)  # (128,128,3)
    img = img.transpose((2, 0, 1))  # (3,128,128)

    img = img[(2, 1, 0), :, :] / 255.0  #(3,128,128)
    img = np.expand_dims(img, axis=0)   #(1, 3, 128, 128)
    return img
...
def predict(predictor, image, vel):
    img = image  # 输入图像数(1, 3, 128, 128)

    speed = [[float(vel)] * 3 * 128]  # 转为（1，10）尺寸
    speed = np.array(speed)  # list转为arrary
    speed = speed.reshape(1, 3, 1, 128)

    img_speed = np.concatenate((img, speed), axis=2)

    input = predictor.get_input(0)  # 接口参数名称0
    input.resize((1, 3, 129, 128))  # 设置接口图像大小
